...
describe('TrackRow', () => {
  it('displays the track label with correct number', () => {
    render(<TrackRow track={makeTrack({ index: 0 })} onToggleMute={vi.fn()} onSetVolume={vi.fn()} />)
    expect(screen.getByText(/track 1/i)).toBeInTheDocument()
  })

  it('displays correct number for track index 2', () => {
    render(<TrackRow track={makeTrack({ index: 2 })} onToggleMute={vi.fn()} onSetVolume={vi.fn()} />)
    expect(screen.getByText(/track 3/i)).toBeInTheDocument()
  })

  it('shows recording indicator when state is RECORDING', () => {
    render(<TrackRow track={makeTrack({ state: TrackState.RECORDING })} onToggleMute={vi.fn()} onSetVolume={vi.fn()} />)
  ...
  it('shows playing indicator when state is PLAYING', () => {
    render(<TrackRow track={makeTrack({ state: TrackState.PLAYING })} onToggleMute={vi.fn()} onSetVolume={vi.fn()} />)
    const indicator = screen.getByTestId('track-state-indicator')
    expect(indicator.classList.contains('playing')).toBe(true)
  })

  it('shows muted indicator when state is MUTED', () => {
    render(<TrackRow track={makeTrack({ state: TrackState.MUTED })} onToggleMute={vi.fn()} onSetVolume={vi.fn()} />)
    const indicator = screen.getByTestId('track-state-indicator')
  ...
  it('calls onToggleMute when mute button clicked', async () => {
    ...
    render(<TrackRow track={makeTrack()} onToggleMute={onToggleMute} onSetVolume={vi.fn()} />)
  ...
  it('mute button shows "Mute" when playing and "Unmute" when muted', () => {
    const { rerender } = render(
      <TrackRow track={makeTrack({ state: TrackState.PLAYING, isMuted: false })} onToggleMute={vi.fn()} onSetVolume={vi.fn()} />
    )
    expect(screen.getByRole('button', { name: /^mute$/i })).toBeInTheDocument()

    rerender(
      <TrackRow track={makeTrack({ state: TrackState.MUTED, isMuted: true })} onToggleMute={vi.fn()} onSetVolume={vi.fn()} />
    )
